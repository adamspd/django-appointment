...
- slot filtering, so an unavailability is honoured by the booking page, the reschedule page and the
  next-available-date lookup alike. It is applied by the same pass that removes slots taken by existing
  appointments, so the service's real duration and `slot_gap_time` are respected around it too.
