...
`appointments.css`, `appointments-user-details.css`, `thank_you.css`, `app_admin/admin.css`, `appt-common.css` and
`app_admin/display_appointment.css` are no longer used by the package pages (they use `djappt.css` and a stylesheet
per page). They are kept for overrides that still link them and will be removed in a
