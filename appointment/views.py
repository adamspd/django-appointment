...
def get_available_slots_ajax(request):
    ...
    if not slot_form.is_valid():
        ...
        return json_response(message=message, custom_data=custom_data, success=False, error_code=error_code)
...
def get_next_available_date_ajax(request, service_id):
    ...
    if staff_id and staff_id != "none":
        ...
        days_off = DayOff.objects.filter(staff_member=staff_member).filter(
            Q(start_date__lte=date.today(), end_date__gte=date.today()) | Q(start_date__gte=date.today())
        )
...
def appointment_request_submit(request):
    ...
    EDU: CUando le damos a NExt en un request aparecemos aqui
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not staff_exists:
                ...
                # All hardcoded, we don't want to set a client, it's all internal always
                appointment_data = {"phone": "", "want_reminder": False, "address": "", "additional_info": ""}
                client_data = {
                    "name": "Llirka",
                    "email": "llirka@llirka.com",
                }
                ...
                request.session.setdefault(f"appointment_submitted_{ar.id_request}", True)
                return response
        else:
...
#             client_data = {'name': 'Llirka', 'email': 'llirka@llirka.com'}
...
def prepare_reschedule_appointment(request, id_request):
    ...
    staff_filter_criteria = (
        {"id": ar.staff_member.id} if not staff_change_allowed_on_reschedule() else {"services_offered": ar.service}
    )
